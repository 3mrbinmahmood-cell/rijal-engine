"""Rebuild Shamela Reader V0.6.1 from the V0.6.0 ZIP.

Only program files in shamela_v061 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v061')
...
def build(source,output):
    ...
    replacement={p.relative_to(OVERLAY).as_posix():p for p in OVERLAY.rglob('*')
                 if p.is_file()}
    if not {'sanad-core.js','full-corpus.js'}<=replacement.keys():
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusSanad' not in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.6.0 reader input')
    ...
    with ZipFile(source) as original,ZipFile(output) as result:
        for name in ('data.js','toc.js','rijal-data.js','reader.js','index.html'):
...
def main():
    ...
    p.add_argument('v060_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v060_zip,a.output_zip))
