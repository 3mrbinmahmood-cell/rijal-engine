"""Rebuild Shamela Reader V0.5.9 from the V0.5.8 ZIP.

Only program files in shamela_v059 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v059')
...
def build(source,output):
    ...
    replacement={p.relative_to(OVERLAY).as_posix():p for p in OVERLAY.rglob('*')
                 if p.is_file()}
    if not {'full-corpus.js'}<=replacement.keys():
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusForName' not in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.5.8 reader input')
...
def main():
    ...
    p.add_argument('v058_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v058_zip,a.output_zip))
