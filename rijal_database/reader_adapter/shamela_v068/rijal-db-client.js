...
class RijalDatabaseClient{
 ...
 async request(path,params={},signal){const url=new URL(this.base+'/api/v1/'+path);for(const [k,v]of Object.entries(params))if(v!==undefined&&v!==null&&v!=='')url.searchParams.set(k,v);let response;try{response=await fetch(url,{signal})}catch(e){if(e.name==='AbortError')throw e;throw Error('شغّل قاعدة الرجال المحلية أولًا: START_WINDOWS.bat')};let result;try{result=await response.json()}catch{throw Error('خدمة الرجال على '+this.base+' لم تُرجع JSON؛ تحقق من START_WINDOWS.bat ومن المنفذ 8765')}if(!response.ok)throw Error(result.error||'تعذر الاتصال بقاعدة الرجال');return result}
