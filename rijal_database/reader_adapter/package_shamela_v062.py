"""Rebuild Shamela Reader V0.6.2 from the V0.6.1 ZIP.

Only program files in shamela_v062 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v062')
...
def build(source,output):
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusSanad' not in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.6.1 reader input')
    ...
    with ZipFile(source) as original,ZipFile(output) as result:
        for name in ('data.js','toc.js','rijal-data.js','reader.js','index.html','sanad-core.js'):
...
def main():
    ...
    p.add_argument('v061_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v061_zip,a.output_zip))
