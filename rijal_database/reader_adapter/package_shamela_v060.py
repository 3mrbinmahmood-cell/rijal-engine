"""Rebuild Shamela Reader V0.6.0 from the V0.5.9 ZIP.

Only program files in shamela_v060 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v060')
...
def build(source,output):
    ...
    replacement={p.relative_to(OVERLAY).as_posix():p for p in OVERLAY.rglob('*')
                 if p.is_file()}
    if not {'reader.js','sanad-core.js','full-corpus.js','index.html'}<=replacement.keys():
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusForName' not in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.5.9 reader input')
    ...
    with ZipFile(source) as original,ZipFile(output) as result:
        for name in ('data.js','toc.js','rijal-data.js'):
...
def main():
    ...
    p.add_argument('v059_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v059_zip,a.output_zip))
