...
(async()=>{
 ...
 let ready,searches=0,identities=0,pages=0,evidence=0;
 ...
 const root={RijalDatabaseClient:class{
  ...
  async graphEvidence(){evidence++;return{results:[{entry_id:'entry-1',page_id:'graph-page'}],has_more:false}}
 ...
 const item=graph.children[2].children[1];await item.children[1].events.click();await new Promise(resolve=>setImmediate(resolve));assert.equal(pages,2);
