## V0.6.2 — 24 September 2026

- Ranked entries headed by the searched name before incidental mentions, including Companion-name searches.
- Prioritized available Companion sources such as Sullam al-Wusul when a Taqrib entry says Companion; reported when Sullam is absent.
- Preserved the distinction between name evidence and verified identity.

## V0.6.1 — 24 September 2026

- Prioritized explicit Taqrib narrator assessment wording, then Ibn Hajar and al-Mizzi source entries in full-database name and sanad results.
- Expanded sanad opening markers and prevented page prose before a chain from swallowing the first narrator.
- The displayed assessment remains a source quote on a candidate entry, never an automatic grade of the hadith.

## V0.6.0 — 24 September 2026

- Routed the visible narrator-name and chain screens through the full Rijal Database service.
- Removed the active local three-book grading and relationship shortcut; the chain parser extracts names only.
- Shows candidate source entries for each name and requires manual identity and source review; no automatic hadith grade.
- Preserved reader text, notes, and bundled source files.
