# Shamela Reader V0.6.2 — start and upgrade
...
## V0.5.8 source evidence

A three-book Rijal profile has a button to search its heading in the full database. Select a result to inspect its identity status; the name search does not merge people. Date claims open their supporting page. Teacher/student mentions open their evidence locations and then the full source page. Start the local API with the optional identity, date and graph databases as described above. Export your annotations before extracting the new ZIP; the storage format is unchanged.

## V0.5.9 relationship exploration

The full-corpus detail can page through linked source entries and supporting relationship pages. **هويات أخرى تذكر الاسم** lists other provisional identities that mention the same unresolved name, without claiming they refer to one person. Open their source entries for review. The local API and optional identity and graph files must be running. Export a backup before extracting V0.5.9; annotation storage remains unchanged.

## V0.6.0 full database name and sanad check

Start Rijal Database V1 on port 8765 before opening the name search or **افحص السند في قاعدة الرجال**. The chain parser extracts up to twelve names from the selected text or current page, then searches source entries for each name. Choose a candidate to inspect its original text and optional identity, date and relationship evidence. A matching name does not identify a narrator, establish a teacher link, or grade a hadith. The old three-book shortcut is removed from the active reader. The bundled books remain for reading. If the service is unavailable the error is shown without silently reverting to the old check. Export your reader backup before extracting this version; annotation storage is unchanged.

## V0.6.1 ordered candidate evidence

Name and sanad searches separately query the three priority works where available: *Taqrīb al-Tahdhīb*, *Tahdhīb al-Tahdhīb* (including supplied editions), and *Tahdhīb al-Kamāl*. An explicit assessment phrase in a Taqrīb entry title appears first; the other two sources follow before general results. Only Taqrīb title wording receives the automatic assessment label. A source quote on a matching name does not prove that it belongs to the narrator in this chain. Search results are bounded; narrow names when additional matches exist. The parser recognizes more opening formulas and starts at the first strong chain marker in a page. Export annotations before extracting the new ZIP; the storage format is unchanged.

## V0.6.2 Companion sources and name headings

Entries headed by a searched name precede incidental mentions, as in an عائشة search that also finds entries about another person who quotes her. If Taqrīb labels a candidate صحابي or صحابية, available Companion sources are prioritized, including *Sullam al-Wusul ilā Ṭabaqāt al-Fuḥūl* when installed. The reader reports when that work is absent; it does not invent its contents or conclude shared identity. To update the separately installed V1 database search page, apply the accompanying small database search update and restart the service.
