...
(async()=>{
 ...
 const root={ShamelaSanad:{parse:()=>['أحمد بن محمد']},RijalDatabaseClient:class{
  async books(){return[{id:'taqrib',title:'تقريب التهذيب'},{id:'tahdhib',title:'تهذيب التهذيب'},{id:'mizzi',title:'تهذيب الكمال في أسماء الرجال'}]}
  async search(q,{book}={}){searches++;assert.equal(q,'أحمد بن محمد');const grade={id:'entry-1',title:'أحمد بن محمد ثقة',citation:{title:'تقريب التهذيب'}};const generic={id:'entry-2',title:'أحمد بن محمد',citation:{title:'كتاب الرجال'}};return{results:book==='taqrib'?[grade]:book?[]:[generic,grade],has_more:false}}
 ...
 assert.equal(searches,4);assert.equal(nodes.corpusResults.children.length,2);assert(nodes.corpusResults.children[0].textContent.includes('عبارة حكم'));
 ...
 root.showFullCorpusSanad('حدثنا أحمد بن محمد',true);await new Promise(resolve=>setImmediate(resolve));assert.equal(nodes.corpusResults.children.length,1);assert(nodes.corpusResults.children[0].children.some(x=>x.textContent.includes('مداخل مرشحة')));assert(nodes.corpusStatus.textContent.includes('الأول: أحمد بن محمد'));assert.equal(searches,8);
