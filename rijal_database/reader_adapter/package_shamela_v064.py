"""Rebuild Shamela Reader V0.6.4 from the V0.6.3 ZIP.

Only program files in shamela_v064 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v064')
...
def build(source,output):
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusSanad' not in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.6.3 reader input')
...
def main():
    ...
    p.add_argument('v063_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v063_zip,a.output_zip))
