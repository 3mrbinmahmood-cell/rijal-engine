"""Rebuild Shamela Reader V0.5.8 from the V0.5.7 ZIP.

Only program files in shamela_v058 are overlaid. The large source data and
...
OVERLAY=Path(__file__).with_name('shamela_v058')
...
def build(source,output):
    ...
    replacement={p.relative_to(OVERLAY).as_posix():p for p in OVERLAY.rglob('*')
                 if p.is_file()}
    if not {'reader.js','full-corpus.js'}<=replacement.keys():
    ...
    with ZipFile(source) as original:
        ...
        if 'قاعدة الرجال الكاملة' not in original.read('index.html').decode('utf-8') or 'showFullCorpusForName' in original.read('reader.js').decode('utf-8'):
            raise ValueError('Expected V0.5.7 reader input')
...
def main():
    ...
    p.add_argument('v057_zip');p.add_argument('output_zip')
    a=p.parse_args()
    print(build(a.v057_zip,a.output_zip))
