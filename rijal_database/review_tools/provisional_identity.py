...
CREATE TABLE provisional_links(
 ...
 shared_long_statements INTEGER NOT NULL,
 PRIMARY KEY(left_entry_id,right_entry_id));
"""

def build(inventory_path, candidates_path, output_path):
    ...
    try:
        ...
        for a,b,name,support,decision in candidates.execute('''SELECT left_entry_id,
            ...
            if decision in ('different','uncertain'):
                continue
            links.append((a,b,support));union(a,b)
        ...
        for representative,entry_ids in components.items():
            ...
            link_count=sum(a in entry_ids and b in entry_ids for a,b,_ in links)
        ...
        return {'source_entries':len(names),'name_buckets':len(name_rows),
                ...
                'supported_links':len(links),'reviewed_person_ids_created':0}
    finally:
...
def main():
    ...
    a=p.parse_args()
    print(json.dumps(build(a.inventory,a.candidates,a.output),indent=2))
